...
mapping_dict = {'high-extreme': 6, 'extreme': 5, 'medium': 2, 'medium-high': 3, 'low': 0, 'high': 4, 'low-medium': 1}
# mapping_dict = {'high-extreme': 0, 'extreme': 0, 'medium':0, 'medium-high': 0, 'low': 1, 'high': 0, 'low-medium': 0}
classes_counts = {'high-extreme': 0, 'extreme': 0, 'medium': 0, 'medium-high': 0, 'low': 0, 'high': 0, 'low-medium': 0}
...
for item in data['csds_objects']:
    if item['intensity']:
        if mapping_dict[item['intensity']] > -1:
            X.append([item['text'], item['head']])
            y.append(mapping_dict[item['intensity']])
            classes_counts[item['intensity']] += 1

path = "/content/drive/My Drive/csds_storage/"
for cl in mapping_dict.keys():
    if mapping_dict[cl] > -1:
        # Loading the saved JSON file.
        with open(path + cl + '_data_aug.json') as json_file:
            data_aug = json.load(json_file)

        x_data = data_aug['X']
        y_data = data_aug['y']
        for i in range(len(x_data)):
            X.append([x_data[i][0], x_data[i][1]])
            y.append(mapping_dict[y_data[i]])
            classes_counts[y_data[i]] += 1
...
for k in mapping_dict.keys():
    if mapping_dict[k] > -1:
        possible_classes.add(mapping_dict[k])
...
# model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=len(possible_classes))

def print_performance_measures(results):
    # Extract metrics
...
def show_confusion_matrix(confusion_matrix, possible_classes=None, save_path=''):
    ...
    if possible_classes == None:
        ...
        plt.savefig(path + 'confusion_matrix.png', dpi=700, bbox_inches='tight')

    plt.ylabel('True')
    plt.xlabel('Predicted');


def k_fold_cross_validation_train(X, y, num_folds, possible_classes, model_name='bert-base-cased'):
    X = np.array(X)
    y = np.array(y)
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        ###
        model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=num_possible_classes)
        X_train_tokenized = tokenizer(X_train, X_p_train, padding=True, truncation=True, max_length=MAX_LENGTH)
        X_val_tokenized = tokenizer(X_val, X_p_val, padding=True, truncation=True, max_length=MAX_LENGTH)
        train_dataset = Dataset(X_train_tokenized, y_train)
        val_dataset = Dataset(X_val_tokenized, y_val)
        training_args = get_training_arg()
        #
        trainer = Trainer(
            model=model,
            args=training_args,
    ...
    # Get predictions
    encoded_input = tokenizer(X_val, X_p_val, max_length=MAX_LENGTH, padding=True, truncation=True, return_tensors='pt')
...
def train_test_split_train(X, y, possible_classes, model_name='bert-base-cased', test_size=0.33, show_labels=False):
    X_tr, X_va, y_train, y_val = train_test_split(X, y, test_size=test_size, random_state=42, shuffle=True, stratify=y)
    X_train = []
    X_p_train = []
    for item in X_tr:
        X_train.append(item[0])
    ...
    print("Done training")
    results = trainer.evaluate()
    print_performance_measures(results)
    ###
    ...
    df_cm = pd.DataFrame(cm, index=possible_classes, columns=possible_classes)
    if show_labels:
        show_confusion_matrix(df_cm, possible_classes=possible_classes)
    else:
        show_confusion_matrix(df_cm)
    return model


model = train_test_split_train(X, y, possible_classes_labels, model_name=model_name, show_labels=True)

class_labels = list(classes_counts.keys())
class_counts = list(classes_counts.values())
fig = plt.figure(figsize=(12, 5))

# creating the bar plot
plt.bar(class_labels, class_counts, color='orange', width=0.4)

plt.xlabel("Labels of classes")
plt.ylabel("Count of data items")
plt.title("Count of data items in different classes of intensity")
plt.show()